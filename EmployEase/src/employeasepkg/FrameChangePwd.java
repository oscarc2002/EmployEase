    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        if(verifyFields()){
            ...
            try {
                ...
                st.setInt(1, idUser);
                Integer pwdHs=pwdOld.hashCode(); // <-- Modified
                st.setString(2, pwdHs.toString()); // <-- Modified Line
                rs = st.executeQuery();

                if (rs.next()) {
                    query = "UPDATE `empleados` SET `password`= ? WHERE `id_user` = " + idUser;
                    st = connection.getConectarDB().prepareStatement(query);
                    Integer passHs=pwd.hashCode(); //<-- NewLine
                    st.setString(1, passHs.toString()); //<-- NewLine
